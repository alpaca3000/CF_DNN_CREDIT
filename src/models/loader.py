...
def _load_decision_threshold(dataset_name: str) -> float:
    """Đọc ngưỡng quyết định đã tối ưu cho embed_mlp từ eval_results.json."""
    eval_path = OUTPUTS_DIR / dataset_name / "models" / "eval_results.json"
    if not eval_path.exists():
        # throw exception file không tồn tại
        raise FileNotFoundError(f"Không tìm thấy file eval_results.json tại: {eval_path}")
